...
$(function () {
    //下方自動加入控制圓鈕
    // const FullCarousal = document.getElementsByClassName('Full')
    const total = document.querySelectorAll('.Full .carousel-item').length
    // console.log(total);
    append_li();
    function append_li() {
        let li = "";
        ...
        for (var i = 0; i <= total - 1; i++) {
            if (i == (ac) / 2) {
                li += `<button type="button" data-bs-target="#carouselExampleIndicators" class="active" data-bs-slide-to="${i}" class="active" aria-current="true"></button>`;
            } else {
                li += `<button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="${i}"></button>`;
            }
        }
